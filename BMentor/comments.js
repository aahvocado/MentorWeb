...
$(function() {
 // Handler for .ready() called.
	...
	$('#registration_form').load('registration-form.html', function() {
		$('#registration-form').trigger("create");
	});

	$(document).on('click', '#reg_submit_button', function() {
		console.log("Reg Button");
		console.log($('#first_name')[0].value);
		$.ajax({
			url: "api/register",
			dataType: "json",
	        async: false,
...
/*	//Bind the submit button to registation form
	$('#reg_submit_button').bind('click', function() {
		console.log("Reg Button");
		$.ajax({
			url: "api/register",
			dataType: "json",
	        async: false,
			data: {'firstName': $('#first_name')[0].value,
						 'lastName': $('#last_name')[0].value,
						 'email': $('#email')[0].value,
						 'phoneNumber': $('#phone_number')[0].value,
						 'commMethod': $('#comm_method')[0].value
...
*/
	...
	$('#users_page').bind('pagebeforeshow', function (event, ui) {
	...
	$('#list_comments_page').bind('pagebeforeshow',function(event, ui){
		...
		if (user) {
			var api_url = "api/user/" + user + "/comment";
		}
	...
	//Bind to the create so the page gets updated with the listing
	$('#list_user_comments_page').bind('pagebeforeshow',function(event, ui){
		console.log('pagebeforeshow');
	
		//Remove the old rows
		$( ".user_comment_list_row" ).remove();
		var user = $.url().fparam("user_id");

		//JQuery Fetch The New Ones
		$.ajax({
			url: "api/user/" + user + "/comment",
			dataType: "json",
	        async: false,
	        success: function(data, textStatus, jqXHR) {
				console.log(data);
	        	//Create The New Rows From Template
	        	$( "#user_comment_list_row_template" ).tmpl( data ).appendTo( "#user_comments_list" );
	        },
	        error: ajaxError
		});
		
		$('#user_comments_list').listview('refresh');
	});
	...
	//Cleanup of URL so we can have better client URL support
	$('#list_user_comments_page').bind('pagehide', function() {
